...
if [[ ! -z "$LETS_ENCRYPT_DOMAIN" ]]; then
  ...
  certbot --expand --apache -n -d $domainsArg $emailArg --agree-tos -v --logs-dir "$logs"
...
# Setup crontab
env | grep -E "TERM|DB_(ENGINE|APP_|NAME)|RABBITMQ_HOST|DOC|VDR" >> /etc/environment
